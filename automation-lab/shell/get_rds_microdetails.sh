...
instances=$(aws rds describe-db-instances --region ap-south-1 --query 'DBInstances[?Engine==`postgres`].[DBInstanceIdentifier,Endpoint.Address,Endpoint.Port,MasterUsername,AllocatedStorage,DBInstanceClass]' --output text | grep -v 'zeta-pcidss')

# Header
echo "RDSInstance Identifier | DBInstanceClass | AllocatedStorage(GB) | Size_Cluster | DB Name | DB Size | Table Name | Table Size"

# Iterate over each RDS instance
while IFS=$'\t' read -r instance_name endpoint port username allocatedstorage instance_class; do
...
EOF
    ...
    if [ -z "$size_cluster" ]; then
       echo "$instance_name | $instance_class | $allocatedstorage | - | No databases found | - | - | -"
        continue
    fi
...
EOF
    ...
    if [ -z "$db_sizes" ]; then
       echo "$instance_name | $instance_class | $allocatedstorage | $size_cluster | No databases found | - | - | -"
        continue
    fi
...
EOF
        ...
        if [ -z "$table_sizes" ]; then
            echo "$instance_name | $instance_class | $allocatedstorage | $size_cluster | $db_name | $db_size | No tables found | -"
            continue
        fi
        ...
        while IFS=$'|' read -r schema_name table_name table_size; do
            ...
            echo "$instance_name | $instance_class | $allocatedstorage | $size_cluster | $db_name | $db_size | $schema_name.$table_name | $table_size"
        done <<< "$table_sizes"
